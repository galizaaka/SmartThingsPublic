...
def selectRule() {
	dynamicPage(name: "selectRule", title: "Select Conditions, Rule and Results", uninstall: true, install: true) {
		...
		section(title: "More options", hidden: hideOptionsSection(), hideable: true) {
			...
			paragraph "With Physical Overide, Physical ON disables rule evaluation.  OFF re-enables rule evaluation.  Use any of Switches On for True for override."
			input "physicalOverride", "bool", title: "Physical override?", required: false, defaultValue: false
   		}    
	}
}
...
def getState(myCapab, n) {
	...
	else if(myCapab == "Temperature") 	result = input "state$n", "number", title: "Temperature", range: "*..*"
...
//        state.msgFalse = ""
		section("") {
			...
			if(thermoFalse) {
				...
				input "thermoSetHeatFalse", "number", title: "Set heating point", multiple: false, required: false, submitOnChange: true
				input "thermoSetCoolFalse", "number", title: "Set cooling point", multiple: false, required: false, submitOnChange: true 
...
def initialize() {
	...
	if (physicalOverride) subscribe(onSwitchTrue, "switch", switchHandler)
	state.success = null
...
def switchHandler(evt) {
	state.override = evt.isPhysical() && evt.value == "on"
	if (state.override) unschedule()
	else runRule(false)
}
...
private getOverOk() {
	def result = !state.override
//	log.trace "overOk = $result"
	return result
}
